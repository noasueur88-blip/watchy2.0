...
@app.route("/api/guilds/debug")
@login_required
def api_guilds_debug():
    guilds = discord_request("users/@me/guilds", token=session.get("access_token"))
    if not isinstance(guilds, list):
        return jsonify({"error": "pas de guilds", "raw": guilds})
    return jsonify([{
        "id": g.get("id"),
        "name": g.get("name"),
        "owner": g.get("owner"),
        "permissions": g.get("permissions"),
        "permissions_int": int(g.get("permissions") or 0),
        "is_admin": bool(int(g.get("permissions") or 0) & 0x8),
    } for g in guilds])
